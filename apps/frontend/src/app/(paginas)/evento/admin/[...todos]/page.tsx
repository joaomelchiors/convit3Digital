...
export default function PaginaAdminEvento(props: PaginaAdminEventoProps) {
    ...
    const id = params.todos[0]; //pega o id
    ...
    const listaConvidadoConfirmado: Convidado[] =
        evento?.listaDeConvidados.filter((e) => e.confirmado) ?? [];
    const listaConvidadoSemConfirmacao: Convidado[] =
        evento?.listaDeConvidados.filter((e) => !e.confirmado) ?? [];

    const totalConfirmado: number = listaConvidadoConfirmado.reduce(
        (acc: number, convidado: Convidado) => acc + convidado.qtdeAcompanhantes + 1,
        0
    );
